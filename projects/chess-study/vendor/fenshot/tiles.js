...
function sampleClamped(img, x, y) {
...
export function extractBoardImage(img, corners) {
    ...
    for (let ty = 0; ty < BOARD_PX; ty++) {
        ...
        for (let tx = 0; tx < BOARD_PX; tx++) {
            ...
            const wx = sx - fx;
            const v = sampleClamped(img, fx, fy) * (1 - wx) * (1 - wy) +
...
export function extractTiles(img, corners) {
