...
function houghResponse(grad, width, height, axis) {
...
function nonmaxSuppress(arr, winsize = 5) {
        ...
        for (let j = i + 1; j < seq.length; j++) {
            ...
            for (const prev of seqs) {
                for (let k = 0; k < prev.length - 1; k++) {
                    if (seq[i] === prev[k] && seq[j] === prev[k + 1])
                        duplicate = true;
                }
            }
            if (duplicate)
...
function trimSequence(seq, vals) {
    let s = seq.slice();
    let v = vals.slice();
    if (s.length > 9) {
        while (s.length > 7) {
...
function checkerboardScore(img, x0, y0, x1, y1) {
    const w = x1 - x0;
    const h = y1 - y0;
    if (w <= 0 || h <= 0)
        return 0;
...
function rankedPeakSequences(hough) {
    ...
    for (const { candidate } of scored) {
        if (!unique.some((u) => u.full.length === candidate.full.length && u.full.every((v, i) => v === candidate.full[i]))) {
            unique.push(candidate);
        }
        if (unique.length >= MAX_CANDIDATE_SEQS)
            break;
    }
    return unique;
}
 ...
 */
export function snapCorners(img, box) {
 ...
 */
function reconstructSquareBoard(img, good, axis) {
    ...
    for (let k = 0; k + 7 <= good.length; k++) {
        const e = [Math.round(good[k]) - pad, Math.round(good[k + 6]) + pad];
 ...
 *  winner. */
function reconstructFromCandidates(img, candidates, axis) {
    let best = null;
    let bestScore = -Infinity;
...
export function findChessboardCorners(img) {
    ...
    const subX = [];
    for (let k = 0; k + 7 <= linesX.length; k++)
        subX.push(linesX.slice(k, k + 7));
    const subY = [];
    for (let k = 0; k + 7 <= linesY.length; k++)
        subY.push(linesY.slice(k, k + 7));
    let best = null;
    let bestScore = -Infinity;
